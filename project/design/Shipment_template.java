...
//	        para6.getFormat().setAfterSpacing(12f);	        
	        ...
	        String[][] data2 =
	                {
	                        ...
	                        new String[]{"\t635 Booth St","Date:4-15-2020"},
	        ...
	        TableRow row = table2.getRows().get(0);
	        row.setHeight(50);
	        //写入剩余组内容到表格，并格式化数据
...
//	            dataRow.getRowFormat().setBackColor(Color.white);
	            for (int c = 0; c < data2[r].length; c++) {
	                ...
	                if(r <=1 ) 
                	{
	                	range2.getCharacterFormat().setFontSize(7f); 
		                if(c == data2[0].length-1)
...
//	        table2.autoFit(AutoFitBehaviorType.Auto_Fit_To_Contents);
	        table3.getTableFormat().getBorders().setBorderType(BorderStyle.Single);
	        //保存文档
